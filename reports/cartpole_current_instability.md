...
### Preregistered recovery-value representability diagnostic

The uniform-replay result leaves two distinct explanations for the confidently
wrong posterior critic. The target checkpoint may still encode the physical
variables needed for recovery, while its self-generated value objective fits
the wrong scalar function. Alternatively, the policy-conditioned posterior
coordinates themselves may no longer expose enough recovery information for
any ordinary value head to generalize. The earlier generic supervised probe
used random-policy states and constant-action returns; it does not answer this
question on the exact induced recovery boundary of the current reference
stack.

- **Question:** on fixed histories generated by the matched update-2,000 source
  actor, can a fresh critic recover real successor-state values from frozen
  posterior features at the uniform run's solved update 2,300 and collapsed
  final update 3,500?
- **Dataset:** collect up to 4,096 base states from complete source-policy
  episodes beginning at reset seed 17. For both candidate first actions, step
  the real environment once, update each target checkpoint's posterior with
  the real successor observation and candidate action, and compute exact
  discounted continuation returns for two frozen continuation policies: the
  target checkpoint's deterministic actor and a fixed true-state CartPole
  heuristic. Keep candidate pairs and whole source episodes in the same
  train/test partition. Terminal first actions have their exact one-step value
  and do not require a successor-head prediction.
- **Paired fits:** for each checkpoint and continuation policy, fit a fresh
  copy of the deployed distributional critic architecture on (a) frozen
  successor posterior features and (b) symlogged true successor state. Use one
  split/initialization seed (`17`), an 80/20 whole-episode split, AdamW `1e-3`,
  30 epochs, batch 128, and the checkpoint's unchanged two-hot bins. Do not
  update the encoder, RSSM, actor, or checkpoint critic.
- **Primary readouts:** held-out value correlation/MAE; paired real-Q margin
  correlation; action-ordering balanced accuracy and class counts for the
  fresh latent head, true-state control, and deployed critic. Require at least
  100 held-out actionable pairs and at least 20 examples of each preferred
  action for a balanced-accuracy conclusion; otherwise use regression/margin
  results and mark classification inconclusive.
- **Interpretation:** if the true-state control passes (`>=0.90` value
  correlation and `>=0.80` action balanced accuracy) and the fresh latent head
  materially repairs deployed action ordering (at least `+0.15`, reaching
  `>=0.70`) with value correlation `>=0.80`, select online value-target
  grounding as the proximal defect. If the control passes but the latent head
  remains below `0.65` action balanced accuracy or trails true-state value
  correlation by more than `0.20`, select representation observability/
  generalization. Intermediate results keep both mechanisms open and require a
  direct coordinate-invariance test.
- **Stop rule:** one dataset contract and the two frozen checkpoints. Do not
  tune fit hyperparameters, change the online trainer, or launch a canary from
  this diagnostic alone. Implement equation/data-alignment tests, run focused
  tests plus compile/type checks for the probe, preserve its complete summary,
  and document limitations.

### Recovery-value representability result

The diagnostic implementation is clean source `6167bd0`. Its equation and
paired-ordering tests, a real-checkpoint CPU smoke, the full `228`-test fast
suite, compile, and direct script Pyright all passed. The retained output is
`experiments/2026-07-22_cartpole_recovery_value_representability/`. Each target
checkpoint used the same 4,213 base states from 27 complete update-2,000 source
episodes (8,426 candidate-successor rows), seeds beginning at 17, 30-step
discounted real branches, one fixed whole-episode split, and the preregistered
fit settings. The held-out value split contains 1,636 live successor rows.

For continuation under each target checkpoint's own deterministic actor:

| Target | Fresh latent value corr. / MAE | True-state value corr. / MAE | Actionable pairs; classes 0 / 1 | Fresh latent ordering | True-state ordering | Deployed ordering |
|---|---:|---:|---:|---:|---:|---:|
| Solved best, step 2,300 | 0.920 / 1.093 | 0.996 / 0.311 | 71; 34 / 37 | 0.563 | 0.851 | 0.500 |
| Collapsed final, step 3,500 | 0.972 / 0.913 | 0.982 / 0.742 | 719; 10 / 709 | 0.793 | 0.594 | 0.453 |

The solved checkpoint has balanced classes but only 71 actionable held-out
pairs, below the preregistered 100-pair floor. The collapsed checkpoint has
ample actionable pairs but only ten action-0 labels, below the 20-per-class
floor. Their balanced-accuracy values are therefore descriptive rather than
categorical passes. The regression and margin results remain well powered. At
final, the fresh latent head raises action-margin correlation from the deployed
critic's `-0.039` to `0.461` and reduces mean absolute margin error from
`5.260` to `0.834`. It does this without changing one encoder/RSSM parameter.

The fixed heuristic continuation supplies the required two-class representation
control on the same successors: 501 held-out actionable pairs with 23 action-0
and 478 action-1 labels. At the solved/final checkpoints, fresh latent value
correlations are `0.970 / 0.967`, latent ordering balanced accuracies are
`0.859 / 0.848`, and true-state ordering is `0.954` at both. Thus the collapsed
posterior still exposes enough control-relevant information for the deployed
critic architecture to fit a useful value and action ordering.

This selects online value-target grounding as the proximal defect and rejects
irrecoverable posterior information loss. It does not say the representation
is ideal: at the solved checkpoint, excellent scalar-value correlation
coexists with weak target-policy action-margin ordering, showing that the
small difference between two large state values is much harder than fitting
their absolute levels. But representation capacity cannot explain a deployed
final critic that is negatively correlated while a fresh head on identical
features reaches `0.972` value correlation and positive action-margin
correlation.

The result does **not** re-authorize the previously rejected exact replay-return
auxiliary. That target is generated by historical behavior trajectories, not
the continually changing current actor, and its scale-1 canary never solved.
The useful supervised label here deliberately follows the frozen target actor
after each candidate action. The remaining question is why the source-
conforming imagined/replay lambda objective fails to produce that learnable
function. Before inventing another grounding loss, continue the pinned-source
