...
### Stochastic closed-loop audit and next preregistration

The terminal-risk run exposed a large difference between the deterministic
trainer evaluation and the stochastic policy that generates replay. A direct
100-episode fixed-seed audit gave these mean rewards:
...
The collector samples both posterior latents and actor actions, whereas the
trainer's evaluation uses mode latents and argmax actions. Therefore replay
is generated by a substantially less capable behavior distribution than the
headline evaluation. Stochastic collection is normal Dreamer exploration, so
this is a hypothesis about policy robustness and replay distribution, not yet
a claimed bug.

The next one-seed ablation is preregistered as follows:

- **Causal variable:** `train.collector_policy_mode=argmax`; baseline is the
  current sampled actor action. Posterior latent sampling remains unchanged so
  the intervention isolates action exploration rather than both sources of
  randomness.
- **Frozen contract:** cached-carry v3 CartPole, seed 0, 3,500 updates, batch
  8, sequence 32, burn-in 20, replay ratio 16, 20 deterministic evaluations
  every 100 updates; terminal-risk auxiliary disabled (`0.0`).
- **Primary gate:** deterministic evaluation reaches 450, remains above 300,
  finishes above 400, and best-to-final gap is at most 100.
- **Robustness gate:** the held-out 100-episode audit must report both
  argmax/mode and sample/sample means, with no claim of success if only the
  deterministic path improves. Record action/latent sampling combinations at
  steps 2,000, 3,000, and final.
- **Decision:** improvement across both paths selects replay-distribution
  sensitivity as the next reliability boundary; deterministic-only
