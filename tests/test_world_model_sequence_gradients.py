...
def test_is_first_resets_selected_rows_and_blocks_cross_episode_gradients():
    cfg = Config(
        d_hidden=16,
        num_latents=4,
        rnn_n_blocks=1,
    ...
    first_tokens = encoder(torch.randn(2, cfg.n_observations))
    first_output = world_model(
        first_tokens,
    ...
    second_tokens = encoder(torch.randn(2, cfg.n_observations))
    second_output = world_model(
        second_tokens,
        torch.tensor([[0.0, 1.0], [0.0, 1.0]]),
        is_first=torch.tensor([True, False]),
    )
    second_output[3].sum().backward()

    assert first_h.grad is not None
    assert first_z.grad is not None
    assert first_h.grad[0].norm().item() == pytest.approx(0.0)
