...
def test_reference_collector_includes_reset_observation_with_zero_inputs(monkeypatch):
    monkeypatch.setattr(
        "dreamer.runtime.collector.create_env", lambda *_args, **_kwargs: _FiveStepEnv()
    )
    context = mp.get_context("spawn")
    ...
    assert not collector.is_alive()
    assert len(first[1]) == 4
    assert first[1][:, 0].tolist() == [0.0, 1.0, 2.0, 3.0]
    assert first[2][0].tolist() == [0.0, 0.0]
    assert first[3].tolist() == [0.0, 1.0, 1.0, 1.0]
    assert first[6] == 3
    assert not first[4].any()
    assert len(final[1]) == 2
    assert final[1][:, 0].tolist() == [4.0, 5.0]
    assert final[6] == 2
    assert final[4].tolist() == [False, True]
