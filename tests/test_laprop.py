...
def _legacy_laprop_step(
    ...
    *,
    ...
    second_moment = beta2 * second_moment + (1 - beta2) * gradient.square()
    normalized_gradient = gradient / (second_moment.sqrt() + epsilon)
    momentum = beta1 * momentum + (1 - beta1) * normalized_gradient
    parameter = parameter - learning_rate * momentum
    return parameter, momentum, second_moment
...
def test_laprop_matches_legacy_uncorrected_reference() -> None:
    learning_rate = 0.1
    beta1 = 0.9
    beta2 = 0.999
    epsilon = 1e-20
    ...
    for gradient in gradients:
        parameter.grad = gradient.clone()
        optimizer.step()
        expected, momentum, second_moment = _legacy_laprop_step(
            expected,
            gradient,
            momentum,
            second_moment,
...
def test_laprop_state_uses_configured_legacy_equation() -> None:
    original_parameter = torch.nn.Parameter(torch.tensor([1.0]))
    original = LaProp(
        [original_parameter], lr=0.1, eps=1e-20, bias_correction=False
    )
    original_parameter.grad = torch.tensor([2.0])
    original.step()

    state = copy.deepcopy(original.state_dict())
    assert "bias_correction" not in state["param_groups"][0]
    resumed_parameter = torch.nn.Parameter(original_parameter.detach().clone())
    resumed = LaProp(
        [resumed_parameter], lr=0.1, eps=1e-20, bias_correction=False
    )
    ...
    torch.testing.assert_close(resumed_parameter, original_parameter)
